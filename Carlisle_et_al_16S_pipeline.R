...
classify_impact <- function(outcome) {
  if (is.na(outcome) || grepl("no infection|unrelated|unclear", outcome, ignore.case = TRUE)) {
    return("Unclear")
  } else if (grepl("stop Abx|de-escalation|confirms no infection", outcome, ignore.case = TRUE)) {
    return("Rationalisation")
  } else if (grepl("change of Abx therapy|start Abx|diagnosis confirmed", outcome, ignore.case = TRUE)) {
    return("Escalation")
  } else {
    return("Unclear")
  }
